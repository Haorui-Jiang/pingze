...
├── netlify.toml               # Publish dir, functions dir, per-function timeouts, route rewrites
├── package.json
...
directories, and there is **no build step**:
...
(they are read at runtime by the Serverless functions; `.env` is for local use only and is never bundled):
...
- `结果/supabase-config.js` — contains the real Supabase URL and anon key
  (the repository ships only the `.example.js` template)
